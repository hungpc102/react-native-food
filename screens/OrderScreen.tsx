...
function Pending({navigation}:Props) {
 ...
 const handleToInvoice = (orderId:number, orderStatus:string)=>{
  ...
  const fetchData = async () => {
        const fetchedOrders = await getOrder(orderStatus);
  ...
  const renderOrderItem = ({ item}:{item:Order}) => {
    ...
    <View style={styles.itemOrder}>
      <Pressable onPress={()=> handleToInvoice(item.ORDER_ID, item.ORDER_STATUS)}>
        ...
        <Text style={styles.priceOrder}>{item.TOTAL_PAYMENT.toLocaleString('vi-VN')+'đ'}</Text>
 ...
 const handleConfirmOrder = async(orderId:number, orderStatus:string) => {
  ...
  const renderOrderItem = ({ item}:{item:Order}) => {
    ...
    <View style={styles.itemOrder}>
      ...
      <Text style={[styles.priceOrder, {color:'#999'}]}>{item.TOTAL_PAYMENT.toLocaleString('vi-VN')+'đ'}</Text>
      </Pressable>
    </View>
  )};
